...
test('T34: Beheer - Vertalingen aanpassen', async ({ page }) => {
	...
	await expect(page.getByRole('heading', { name: 'Vertalingen', exact: true })).toBeVisible();
	...
	// Check modal opens
	await expect(page.getByRole('heading', { name: 'Vertaling aanpassen' })).toBeVisible();
	...
	await page
		.locator('div.DraftEditor-editorContainer > div[contenteditable="true"]')
		.fill('Registratie is mislukt, het aanmaken van de gebruiker in de database is mislukt');
	await page.waitForTimeout(1000);
