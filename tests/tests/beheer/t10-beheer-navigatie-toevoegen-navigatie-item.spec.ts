...
test('T10: Beheer - Navigatie, toevoegen navigatie item', async ({ page }) => {
	...
	await page.getByRole('link', { name: 'Hoofdnavigatie Links', exact: true }).click();
	await expect(
		page.getByRole('heading', { name: 'Navigatie balk: Hoofdnavigatie Links', exact: true })
	).toBeVisible();

	// Add item
	await page.getByRole('button', { name: 'Voeg een navigatie-item toe' }).click();
	await expect(
		page.getByRole('heading', { name: 'Hoofdnavigatie Links: item Toevoegen', exact: true })
	).toBeVisible();

	// Fill in form
	// Title
	await page.locator('div.m-menu-edit-form > div:nth-child(2) > div > input').fill('Auto Test');

	// Link
	await page.locator('div.c-select__control', { hasText: 'Content' }).first().click();
	await page.waitForTimeout(1000);
	...
	const url = page.url();
	expect(url).toContain((process.env.TEST_CLIENT_ENDPOINT as string) + 'accepteer-voorwaarden');
	await page.waitForTimeout(1000);
	...
	await page.locator('button[aria-label="Verwijder dit navigatie-item"]').last().focus();
	await page.waitForTimeout(1000);
	await page.locator('button[aria-label="Verwijder dit navigatie-item"]').last().click();
	await expect(
