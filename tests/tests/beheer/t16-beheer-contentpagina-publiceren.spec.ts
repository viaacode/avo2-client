...
test.skip('T16: Beheer - Contentpagina publiceren', async ({ page }) => {
	...
	// Publish
	await page.locator('button[aria-label="Maak de contentpagina publiek / niet-publiek"]').click();
	await page.waitForTimeout(1000);

	// Check modal opens
	await expect(
		page.getByRole('heading', { name: 'Maak deze contentpagina publiek.', exact: true })
	).toBeVisible();
	...
	await expect(
		page.locator('div > div.Toastify__toast-body > div > div > div.c-alert__message')
	).toContainText('De contentpagina is nu publiek.');
	await page.waitForTimeout(1000);
