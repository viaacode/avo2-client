...
test('T17: Beheer - Contentpagina dupliceren', async ({ page }) => {
	...
	await page.waitForTimeout(1000);
	await page
		.locator('div.c-dropdown__content-open > div > div:nth-child(1)', { hasText: 'Dupliceer' })
		.click();
	...
	await expect(page.getByRole('link', { name: copiedPageTitleInOverview })).not.toBeVisible();
	...
	// Check page is removed
	await expect(
		page.locator('div:nth-child(2) > .Toastify__toast-body > div > div > div.c-alert__message')
	).toContainText('Het content-item is succesvol verwijderd.');
