...
test.skip('T41: Werkruimte - collecties: Detail collectie zet publiek', async ({ page }) => {
	...
	// Check modal opens
	await expect(page.getByRole('heading', { name: 'Maak deze collectie publiek' })).toBeVisible();
	...
	await expect(
		page.locator('div.o-form-group.o-form-group--error > div > div:nth-child(3)')
	).toContainText('De collectie heeft geen beschrijving.');
	await expect(
		page.locator('div.o-form-group.o-form-group--error > div > div:nth-child(4)')
	).toContainText('De collectie heeft geen onderwijsniveau(s).');
	await expect(
		page.locator('div.o-form-group.o-form-group--error > div > div:nth-child(5)')
	).toContainText('De collectie heeft geen thema(s).');
	await expect(
		page.locator('div.o-form-group.o-form-group--error > div > div:nth-child(6)')
	...
	await page.fill('#shortDescriptionId', 'Dit is een korte beschrijving tekst.');

	// Fill in Persoonlijke notities
	await page.fill('#personalRemarkId', 'Dit is een persoonlijke notitie tekst.');
	...
	await expect(
		page.locator("tr:nth-child(1) > td:nth-child(5) > div[title='Niet publiek']")
	).not.toBeVisible();
