...
test('T29: Fragment detail - Voeg toe aan nieuwe opdracht zonder beschrijving', async ({
	...
	await expect(page.getByRole('link', { name: 'Mijn opdrachten' })).toBeVisible();
	...
	// Click 'Verwijderen'
	await page.locator('div.c-dropdown__content-open > div > button:nth-child(3)').click();

	// Check modal opens
	await expect(
		page.getByRole('heading', { name: 'Ben je zeker dat je deze opdracht wil verwijderen?' })
	).toBeVisible();
