...
test('T56: Opdracht - Elk type toevoegen', async ({ page }) => {
	...
	await expect(page.getByRole('heading', { name: 'Toevoegen' })).not.toBeVisible();
	...
	await expect(page.getByRole('heading', { name: 'Voeg collectie toe' })).toBeVisible();
	...
	// Confirm 2x
	await page.getByRole('button', { name: 'Voeg toe' }).click();
	await expect(page.getByRole('heading', { name: 'Knip fragment (optioneel)' })).toBeVisible();
	await page.waitForTimeout(1000);
	await page.getByRole('button', { name: 'Voeg toe' }).click();
	...
	).toContainText('De opdracht is succesvol aangemaakt, je vindt deze in je Werkruimte.');
