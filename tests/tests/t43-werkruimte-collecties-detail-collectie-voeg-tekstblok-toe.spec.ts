...
test('T43: Werkruimte - collecties: Detail collectie voeg tekstblok toe', async ({ page }) => {
	...
	// Check if text block is visible
	await expect(page.getByText('Automatische test titel tekst blok')).toBeVisible();
	await expect(page.getByText('Automatische test beschrijving tekst blok')).toBeVisible();
