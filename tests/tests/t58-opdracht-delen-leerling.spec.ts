...
test.skip('T58: Opdracht - Delen met leerling', async ({ page }) => {
	...
	await page.locator('input[placeholder="Geef een titel in"]').nth(1).fill(assignmentTitle);
	...
	await expect(
		page.getByRole('heading', { name: 'Link nog niet deelbaar' }).getByRole('strong')
	).toBeVisible();

	await page.waitForTimeout(1000);

	// Click outside share modal
	await page.click('div.c-assignment-page.c-assignment-page--edit.c-sticky-bar__wrapper');
	...
	// Enter title
	await page.fill('input[placeholder="Instructies of omschrijving"]', 'Automatische test titel');
	...
	// Check modal opens
	await expect(page.getByRole('heading', { name: 'Beheer je klassen' })).toBeVisible();
	...
	await expect(page.getByRole('link', { name: assignmentTitleInOverview })).toBeVisible();
	...
	await expect(
		page.locator('div.c-block-list__item > div > div > div > div.c-icon-bar__content > h2')
	...
	await page.locator('button[aria-label="Verwijder dit label"]').first().click();
	await page.waitForTimeout(2000);
