...
test.skip('T39: Werkruimte - collecties: Voeg toe aan nieuwe opdracht', async ({ page }) => {
	...
	// Check modal opens
	await expect(page.getByRole('heading', { name: 'Voeg toe aan nieuwe opdracht' })).toBeVisible();
	...
	await expect(page.locator('h2.c-collection-fragment-title')).toContainText('dag van de');
