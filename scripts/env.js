...
console.info('copying env vars to dist/client/env-config.js file...');
const CI_ENV_VARIABLES = {
  ...
  KEYCLOAK_ACCOUNT_EDIT_URL: process.env.SSUM_PASSWORD_EDIT_URL,
  KEYCLOAK_PASSWORD_EDIT_URL: process.env.SSUM_PASSWORD_EDIT_URL,
  USE_KEYCLOAK_INSTEAD_OF_SSUM: process.env.SSUM_PASSWORD_EDIT_URL,
  ...
  ENABLE_LOG_CLIENT_REDIRECTS: process.env.ENABLE_LOG_CLIENT_REDIRECTS,
};
...
Object.keys(envVariables).forEach((envName) => {
  if (
    ...
    Object.keys(CI_ENV_VARIABLES).includes(envName)
  ) {
...
console.info('copying env vars to dist/client/env-config.js file... done');
