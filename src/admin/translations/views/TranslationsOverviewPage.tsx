...
const TranslationsOverviewPage: FC = () => {
  ...
  const convertDataToTranslations = (data: Translation[]) => {
    ...
    return map(
      ...
      (translations: Translation, context: string) => ({
      ...
      return (
        <div className="u-px-32 u-py-24">
          <Button
            onClick={onSave}
            label={tText(
              'pages/admin/vertalingen-v-2/index___bewaar-wijzigingen',
            )}
          />
  ...
  return (
    <PermissionGuard permissions={[PermissionName.EDIT_TRANSLATIONS]}>
      <AdminLayout
        pageTitle={tText(
          'admin/translations/views/translations-overview___vertalingen',
        )}
        size="full-width"
      >
        <AdminLayoutTopBarRight>
