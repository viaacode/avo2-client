...
const UserDetail: FunctionComponent<UserDetailProps> = ({ history, match, user }) => {
	...
	const CONTENT_DROPDOWN_ITEMS: MenuItemInfo[] = [
		createDropdownMenuItem(
			'editInAcm',
			t('admin/users/views/user-detail___beheer-in-account-manager'),
			'external-link'
		),
	...
	const onSetTempAccess = async (tempAccess: TempAccess) => {
		try {
	...
	const renderTempAccess = (tempAccess: TempAccess | null): String => {
		if (!tempAccess) {
			return '-';
		}
	...
	const renderTempAccessDuration = (tempAccess: TempAccess | null): String => {
		if (!tempAccess) {
			return '-';
		}
		const from = get(tempAccess, 'from');
