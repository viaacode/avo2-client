...
const MenuDetail: FunctionComponent<MenuDetailProps> = ({ history, match }) => {
	...
	const handleDelete = (refetchMenuItems: () => void): void => {
		triggerMenuItemDelete({
			...
			.then(() => {
				refetchMenuItems();
				toastService.success(t('Het navigatie item is succesvol verwijderd'), false);
			})
			.catch(err => {
				console.error(err);
				toastService.danger(t('Het verwijderen van het navigatie item is mislukt'), false);
			});
	};
	...
	const handleSave = (refetch: () => void): void => {
		...
		Promise.all(promises)
			.then(() => {
				refetch();
				toastService.success(t('De navigatie items zijn succesvol opgeslagen'), false);
			})
			.catch(err => {
				console.error(err);
				toastService.danger(t('Het opslaan van de navigatie items is mislukt'), false);
			});
	};
	...
	const renderMenuDetail = (menu: Avo.Menu.Menu[], refetchMenuItems: () => void) => {
		...
		if (!menu.length) {
			toastService.danger(t('Er werden geen navigatie items gevonden'));
