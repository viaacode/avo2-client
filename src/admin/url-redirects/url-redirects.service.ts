...
export class UrlRedirectsService {
  public static async fetchUrlRedirectMap(): Promise<{
    ...
    try {
      ...
      const urlRedirects = await fetchWithLogoutJson<{
        [avo1Path: string]: string
      }>(url)
      ...
      const error = new CustomError(
        'Failed to fetch url redirects from database',
        err,
        {
          url,
        },
  ...
  public static async fetchUrlRedirectsOverview(
    params: UrlRedirectFilters,
  ): Promise<{
  ...
  public static async fetchSingleUrlRedirect(
  ...
  public static async insertUrlRedirect(
    ...
    try {
      ...
      const error = new CustomError('Failed to create url redirect', err, {
        url,
        urlRedirect,
        errorMessage: (err as CustomError)?.additionalInfo?.responseBody
  ...
  static async updateUrlRedirect(
    urlRedirect: UrlRedirect,
  ): Promise<UrlRedirect> {
    let url: string | undefined = undefined
