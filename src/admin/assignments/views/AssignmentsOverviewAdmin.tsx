...
export const AssignmentOverviewAdmin: FC = () => {
    ...
    () => [
      {
        id: NULL_FILTER,
        label: tText(
          'admin/assignments/views/assignments-overview-admin___geen-label',
        ),
      ...
      ...(allQualityLabels || []).map(
        (option): CheckboxOption => ({
          ...
          checked: (tableState.quality_labels || []).includes(
            String(option.value),
          ),
        }),
      ),
  ...
  const changeAuthorForSelectedAssignments = async (profileId: string) => {
    ...
    try {
      ...
      console.error(
        new CustomError(
          'Failed to update the author of the selected assignments',
          err,
          {
            tableState,
          },
      ...
      <>
        ...
        <ul className="c-assignment-bulk-warning-being-edited">
          {assignmentsBeingEdited.map((assignmentBeingEdited) => (
            <li
              key={`assignment-being-edited-${assignmentBeingEdited.subjectId}`}
            >
              <a
                target="_blank"
  ...
  return (
    <PermissionGuard permissions={[PermissionName.VIEW_ANY_ASSIGNMENTS]}>
      <AdminLayout
        pageTitle={tText(
          'admin/assignments/views/assignments-overview-admin___opdrachten',
        )}
        size="full-width"
      >
        <AdminLayoutBody>
