...
export const LinkedAccounts: FC = () => {
  ...
  const deleteIdpModals: Record<string, DeleteModalToggle> = {
    ...
    SMARTSCHOOL: {
      open: isDeleteSmartschoolModalOpen,
      setter: setIsDeleteSmartschoolModalOpe,
    },
    LEERID: {
      open: isDeleteLeerIDModalOpen,
      setter: setIsDeleteLeerIDModalOpen,
    },
    KLASCEMENT: {
  ...
  const idpProps: Record<string, IdpProps> = {
    VLAAMSEOVERHEID: {
      ...
      description: isUserAPupil
        ? tHtml(
            'settings/components/linked-accounts___aanmelden-met-je-leer-id',
          )
        : tHtml(
            'settings/components/linked-accounts___itsme-e-id-of-een-digitale-sleutel',
          ),
      iconNames: isUserAPupil
        ? [IconName.leerid]
        : [IconName.itsme, IconName.eid],
      ...
      <Spacer margin="top">
        {!(isUserAPupil && currentIdp.hideForPupil) && (
          <Grid className="c-account-link">
            <Column
              className={`c-account-link__column ${className}`}
              size="3-2"
            >
