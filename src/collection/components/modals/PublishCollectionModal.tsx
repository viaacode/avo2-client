...
interface PublishCollectionModalProps {
  isOpen: boolean;
  onClose: (collection?: AvoCollectionCollection) => void;
...
export const PublishCollectionModal: FC<PublishCollectionModalProps> = ({
  ...
  const onSave = async () => {
    try {
      ...
      const newCollectionProps: Partial<AvoCollectionCollection> = {
  ...
  const closeModal = (newCollection?: AvoCollectionCollection) => {
    setValidationErrors([]);
