...
export function getProfileInitials(commonUser: Avo.User.CommonUser | undefined): string {
	if (!commonUser) {
		throw new CustomError(
...
export function isProfileComplete(commonUser: Avo.User.CommonUser | null): boolean {
	if (!commonUser) {
		return false;
	}
	...
	if (userGroupId === SpecialUserGroupId.TeacherSecondary) {
		return (
			...
			!!commonUser.loms?.find((lom) => lom.lom?.scheme === LomSchemeType.structure) &&
			!!commonUser.loms?.find((lom) => lom.lom?.scheme === LomSchemeType.subject)
		);
	}
