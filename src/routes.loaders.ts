...
export async function fetchContentPageLoader(args: LoaderFunctionArgs<any>) {
	try {
		console.log('Fetching content page for url:', args.request.url);
		// Load content page for the requested path
		const path = new URL(args.request.url).pathname;
		const cookieHeader = args.request.headers.get('cookie');
		console.log('Fetching content page for url:', args.request.url, ' with cookie:', cookieHeader);
		const contentPage =
		...
		if (statusCode === 401 || statusCode === 403) {
			...
			console.error('Content page not accessible for user (403) for url:', args.request.url, ' => ', redirectUrl);
