...
import { GET_QUICK_LANES_BY_OWNER } from './workspace.gql';
...
export class WorkspaceService {
	static async fetchQuickLanesByOwnerId(profileId: string) {
		try {
			...
			if (!urls) {
				throw new CustomError('Quick lane urls do not exist', null, {
					response,
				});
			}
			...
			throw new CustomError(
				'Failed to get quick lane url by  profile id from database',
				err,
				{
					profileId,
					query: 'GET_QUICK_LANES_BY_OWNER',
